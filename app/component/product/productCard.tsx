...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  // Updated handleCardClick function to navigate to the product page dynamically
  const handleCardClick = () => {
    router.push(`/productpage/${product.id}`); // Navigate to the product page with the product ID
  };
  ...
  return (
    <div
      ...
      onClick={handleCardClick} // Navigate to the product page on card click
    >
      ...
      <div className="p-3">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center font-semibold">
            <span
              className={`ml-2 text-[#7b5af7] text-xs flex items-center bg-[#bdadfb] px-3 py-1 rounded-full font-semibold 
              ${product.isDiscounted ? "visible" : "invisible"}`}
            >
              -{product.discount}%
            </span>
            <span className="ml-2 text-xs text-[#5A31F5]">
              Rs {product.price}
            </span>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center mt-2">
          <div className="flex items-center space-x-1">
            <span
              className={`text-red-600 text-[11px] font-semibold italic flex ${
